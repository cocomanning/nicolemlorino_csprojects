...
def find_word(s, hash_table):
    ...
    key_value = hash_word(s, table_size)
    step = step_size(s)

    while hash_table[key_value] != "":
        if hash_table[key_value] == s:
            return True
        key_value = (key_value + step) % table_size
    return False
...
def is_reducible(s, hash_table, hash_memo):
    ...
    """
    if find_word(s, hash_table) is False  or (len(s) == 1 and s != "a" and s != "i" and s != "o"):
        return False
    if s == "a" or s == "i" or s == "o" or find_word(s, hash_memo) is True:
        return True
    for i in range(len(s)):
        s1 = s[:i] + s[i+1:]
        if is_reducible(s1, hash_table, hash_memo):
            index = hash_word(s, 37)
            hash_memo[index] = s
            return True
    return False
...
def get_longest_words(string_list):
    ...
    for x in string_list:
        if len(x) > longest_word:
            longest_word = len(x)
            longest_words = [x]
        if len(x) == longest_word and x not in longest_words:
...
def main():
    ...
    largest_word = get_longest_words(reducible_words)
