...
This program creates an "Evil Wordle" game in which the mystery word changes every guess, 
but the correctly guessed letters and placement are passed onto the new word until there 
is only one viable word remaining.
"""
...
class Keyboard:
    ...
    def update(self, feedback_colors, guessed_word):
        ...
        for index, letter in enumerate(guessed_word):
            feedback = feedback_colors[index]
    ...
    def __str__(self):
        ...
        for row in self.rows:
            ...
            updated_row = ""
            for letter in row:
                updated_row += color_word(self.colors[letter], letter)
                updated_row += " "
            updated_keys += " "*spaces + updated_row + "\n"
            row_number += 1
...
class WordFamily:
    ...
    def __init__(self, feedback_colors, words):
        ...
        for feedback in feedback_colors:
            self.difficulty += self.COLOR_DIFFICULTY[feedback]
    ...
    def __lt__(self, other):
        ...
        if len(self.words) == len(other.words):
            ...
            if self.difficulty == other.difficulty:
                for i in range(5):
                    if self.feedback_colors[i] < other.feedback_colors[i]:
                        return True
                    elif self.feedback_colors[i] > other.feedback_colors[i]:
                        return False
                return False
        return True
...
def fast_sort(lst):
    ...
    for num in lst:
        if num < pivot:
            ...
            more.append(num)
    return fast_sort(less) + equal + fast_sort(more)
...
def get_feedback_colors(secret_word, guessed_word):
    ...
    copy_word = list(secret_word)
    for i in range(NUM_LETTERS):
        if guessed_word[i] == secret_word[i]:
            feedback[i] = CORRECT_COLOR
            copy_word.remove(guessed_word[i])
    for i in range(NUM_LETTERS):
        if feedback[i] != CORRECT_COLOR:
            if guessed_word[i] in copy_word:
...
def get_feedback(remaining_secret_words, guessed_word):
    ...
    for word in remaining_secret_words:
        feedback_color = get_feedback_colors(word, guessed_word)
        feedback_colors.append(feedback_color)
    # make list of all feedback patterns
    pattern_list = []
    for pattern in feedback_colors:
        if pattern not in pattern_list:
            pattern_list.append(pattern)
    # copy of remaining words list
    remaining_words = []
    for word in remaining_secret_words:
        remaining_words.append(word)
    # group words based on pattern
    master_list = []
    for pattern in pattern_list:
        word_pattern_list = []
        word_pattern_list.append(pattern)
        count = 0
        for word in remaining_words:
            if count < len(feedback_colors) and feedback_colors[count] == pattern:
                word_pattern_list.append(word)
            count += 1
        master_list.append(word_pattern_list)
    hardest = 0
    hardest_pattern = 0
    for words in master_list:
        word_family = WordFamily(words[0], words[1:])
        if hardest == 0:
            hardest = word_family
            hardest_pattern = tuple(words[0])
            remaining_words = words[1:]
        elif word_family < hardest:
            hardest = word_family
            hardest_pattern = tuple(words[0])
            remaining_words = words[1:]
    return hardest_pattern, remaining_words
